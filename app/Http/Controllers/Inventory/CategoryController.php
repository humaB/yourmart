...
class CategoryController extends Controller
{
    public function fetchCategories(){
        ...
        $record = Category::with('user', 'parent:id,name')->orderBy('id', 'desc')->get();
    ...
    public function store( Request $request ){
        $lock = Cache::lock('add_category')->block(7, function () use ($request) {
            // Validate request
    ...
    public function update(Request $request) {
        $id = $request->id;
        $lock = Cache::lock('update_category_' . $id)->block(7, function () use ($request, $id) {
            // Validate request
            $validator = \Validator::make($request->all(), [
                'name' => 'required',
            ]);
            ...
            if (!$category) {
                return (new ValidationCollection(['Category not found']))
                ->response()
                ->setStatusCode(404);
            }
