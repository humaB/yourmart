    ...
    <div class="card">
        <div class="card-header d-block pb-0">
            ...
            <div class="row mt-4 px-0">
                <div class="col-md-4">
                    <h5 class="mb-0">General Journals</h5>
                </div>
                <div class="col-md-4 text-center">
                    <h6 class="mb-0">From : {{ journals.from }}</h6>
                </div>
        ...
        <div class="card-body">
            ...
            <div v-else class="table-responsive">
                <table class="table table-sm" id="journal_table">
                    <thead>
                        <tr>
                        ...
                        <tr class="bg-secondary">
                            ...
                            <td class="text-danger">Opening 
                                {{ Math.abs(parseFloat(journals.previous_credits) - parseFloat(journals.previous_debits)) }}
                                ({{ parseFloat(journals.previous_credits) > parseFloat(journals.previous_debits) ? 'Cr' : 'Dr' }})
                            </td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(ledger, index) in journals.record" :key="index">
                            ...
                            <td>{{ ledger.account_head.level_four.name }}</td>
                    ...
                    <tfoot>
                        <tr class="bg-dark">
                            ...
                            <td class="text-white"></td>
                            <td class="text-white"></td>
                            <td class="text-white">{{ journals.debits }}</td>
                            <td class="text-white">{{ journals.credits }}</td>
                            <td class="text-white">
                                {{
                                    Math.abs(
                                        (parseFloat(journals.previous_credits) + parseFloat(journals.credits)) -
                                        (parseFloat(journals.previous_debits) + parseFloat(journals.debits))
                                    )
                                }}
                                ({{ (parseFloat(journals.credits) + parseFloat(journals.previous_credits)) >
                                    (parseFloat(journals.debits) + parseFloat(journals.previous_debits)) ? 'Cr' : 'Dr' }})
                            </td>
                        </tr>
                    </tfoot>
                </table>
...
<script>
export default {
  props: ['btnLoading', 'voucher', 'journals'],
  data() {
        return {
        };
    },
  methods: {
    calculateBalance(index) {
        let previousBalance = parseFloat(this.journals.previous_credits) - parseFloat(this.journals.previous_debits);

        // Iterate over all the ledgers up to the current index to calculate running balance
        for (let i = 0; i <= index; i++) {
            previousBalance += parseFloat(this.journals.record[i].credit) - parseFloat(this.journals.record[i].debit);
        }
