    ...
    <div class="card">
        <div class="card-header d-block pb-0">
            <div class="d-flex justify-content-between">
                ...
                <button
                    @click="printAccountLedgerReport()"
            ...
            <div class="row mt-4 px-0">
                <div class="col-md-4">
                    <h5 class="mb-0">{{ general_ledgers.level_four.name }}</h5>
                </div>
                <div class="col-md-4 text-center">
                    <h6 class="mb-0">From : {{ general_ledgers.from }}</h6>
                </div>
        ...
        <div class="card-body">
            ...
            <div v-else class="table-responsive">
                <table class="table table-sm" id="general_table">
                    <thead>
                        <tr>
                        ...
                        <tr class="bg-secondary">
                            ...
                            <td class="text-danger">Opening 
                                {{ Math.abs(parseFloat(general_ledgers.previous_credits) - parseFloat(general_ledgers.previous_debits)) }}
                                ({{ parseFloat(general_ledgers.previous_credits) > parseFloat(general_ledgers.previous_debits) ? 'Cr' : 'Dr' }})
                            </td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(ledger, index) in general_ledgers.record" :key="index">
                    ...
                    <tfoot>
                        <tr class="bg-dark">
                            ...
                            <td class="text-white"></td>
                            <td class="text-white"></td>
                            <td class="text-white">{{ general_ledgers.debits }}</td>
                            <td class="text-white">{{ general_ledgers.credits }}</td>
                            <td class="text-white">
                                {{
                                    Math.abs(
                                        (parseFloat(general_ledgers.previous_credits) + parseFloat(general_ledgers.credits)) -
                                        (parseFloat(general_ledgers.previous_debits) + parseFloat(general_ledgers.debits))
                                    )
                                }}
                                ({{ (parseFloat(general_ledgers.credits) + parseFloat(general_ledgers.previous_credits)) >
                                    (parseFloat(general_ledgers.debits) + parseFloat(general_ledgers.previous_debits)) ? 'Cr' : 'Dr' }})
                            </td>
                        </tr>
                    </tfoot>
                </table>
...
<script>
export default {
  props: ['btnLoading', 'voucher', 'general_ledgers'],
  data() {
        return {
        };
    },
