              ...
              <div class="col-md-7">
                        ...
                        <div class="view-mail p-t-20">
                                ...
                                <div class="card mb-3">
                                  ...
                                  <div class="card-body">
                                    ...
                                    <p><strong>City:</strong> {{ details.city ? details.city.name : '' }}</p>
                                  </div>
                                </div>
                                ...
                                <div class="card mb-3">
                                  ...
                                  <div class="card-body">
                                    <p><strong>Courier Service:</strong> {{ details.courier_service_id }}</p>
                                    ...
                                    <p><strong>Status:</strong> {{ details.status === 0 ? 'Pending' : 'Completed' }}</p>
                                  </div>
                                </div>
                                    ...
                                    <table class="table table-bordered">
                                      ...
                                      <tbody>
                                        <tr v-for="item in details.items" :key="item.id">
                                          <td>{{ item.variation.product.title }}</td>
                                          <td>{{ item.price }}</td>
                                          <td>{{ item.quantity }}</td>
                          ...
                          <div class="attachment-mail">
                            <p>
                              <span>
                                <i class="fa fa-paperclip"></i> {{ details.attachments ? details.attachments.length : 0 }} attachments — </span>
                              <a href="#">Download all attachments</a>
                            </p>
                            <div class="row" v-if="details.attachments && details.attachments.length > 0">
                              <div class="col-md-2" v-for="item in details.attachments"  :key="item.id">
                                <a target="_blank" :href="setImage(item.attachment)">
                                  <img class="img-thumbnail img-responsive" v-if="isImage(item.attachment)" alt="attachment" :src="setImage(item.attachment)">
                                  <i
                                      v-else
                                ...
                                <a class="name" :href="setImage(item.attachment)" target="_blank"> {{ truncatedAttachmentName(item.attachment) }}
                                </a>
                              </div>
                            </div>
                          </div>
                ...
                <div class="card">
                  <div class="card-body">
                      <div id="accordion">
                          <div class="accordion">
                            <div class="accordion-header" @click="changeLabel()" role="button" data-toggle="collapse" data-target="#panel-body-1" aria-expanded="true">
                              <h4>
                                  <i :class="activityStatus ? 'fas fa-chevron-down' : 'fas fa-chevron-right'"></i>
                            ...
                            <div class="accordion-body collapse" id="panel-body-1" data-parent="#accordion" style="">
                              <ul>
                                  <li class="mb-0" v-for="item in details.activity" :key="item.id">
                                      {{ getStatusMessage(item) }} -  <small class="text-muted">{{ formatDate( item.created_at ) }}</small>
                                  </li>
                              </ul>
                            </div>
                          </div>
                ...
                <div class="card">
                    ...
                    <div class="chat-header clearfix row">
                      <div class="col-md-12 row">
                          <div class="col-md-6">
                              ...
                              <div class="chat-num-messages">{{ details.comments ? details.comments.length : 0 }} Total Comments</div>
                          </div>

                      </div>
                      <div class="col-md-12 mt-2">
                        <form class="card-header-form">
                          <input type="text" name="search"  v-model="searchQuery" class="form-control" placeholder="Search">
                        </form>
                      </div>
                    </div>
                  </div>
                  <div class="chat-box" id="mychatbox">
                    <div class="card-body chat-content">
                      <div v-for="comment in filteredComments"  v-bind:key="comment.id" class="support-ticket media pb-1 mb-3">
                      <img :src="setProfile()" class="user-img mr-2" alt="">
                          <div class="media-body ml-3">
                        <span class="font-weight-bold">{{ comment.comment }}</span><br />
                        <a :href="setCommentImage(comment.attachment)" target="_blank" v-if="comment.attachment">Click to see Attachment</a>
                        <p class="my-1"></p>
                        <small class="text-muted">Comment by <span class="font-weight-bold font-13">{{ comment.user.name }} ( {{ comment.user.role }} )</span>
