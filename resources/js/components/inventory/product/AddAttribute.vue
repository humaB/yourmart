            ...
            <div class="modal-content">
                ...
                <div class="modal-body row">
                    <div class="col-md-12">
                        ...
                        <v-select :options="parentAttributes" v-model="category">
                        </v-select>
                    ...
                    <div class="col-md-12 mt-5 text-right">
                        <button type="button" class="btn btn-primary" v-if="!loader" @click="addNewAttribute()">Add New Attribute</button>
                        <button type="button" class="btn btn-primary btn-progress disabled" v-else>Add New Attribute</button>
                    </div>

                    <div class="col-md-12 mt-5">
                        <label for=""><b>Added Attributes</b></label>
                        <table class="table table-sm">
                            ...
                            <tbody>
                                <tr v-for="(item, index) in attributes" :key="item.id">
                                    <td>{{ index + 1 }}</td>
                                    <td>
                                        <span v-if="!item.editable">{{ item.parent ? item.parent.name : '-' }}</span>
                                        <v-select :options="parentAttributes" v-model="category" v-else>
                                        </v-select>
                                    ...
                                    <td>
                                        <button :class="!item.editable ? 'btn btn-primary' : 'btn btn-success'" :title="!item.editable ? 'edit details' : 'update details'" @click="toggleEdit(item)">
...
export default {
    ...
    mounted() {
        this.$parent.$on("attributeSaved", (value) => {
            if (value) {
                this.close();
            }
        });
    ...
    methods : {
        ...
        addNewAttribute(){
            ...
            if( vm.name == '' ){
                return swal({
                    title: "Required",
                    text: "Please add Attribute Name first, thanks",
                    icon: "error",
                    timer: 3000,
                });
            }
