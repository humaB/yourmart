...
<script>
import NewTransaction from '../../../components/account/transaction/bank-transaction/NewComponent.vue';
import EditTransaction from '../../../components/account/transaction/bank-transaction/EditComponent.vue';
import DetailTransaction from '../../../components/account/transaction/bank-transaction/DetailTransaction.vue';
import ApproveTransaction from '../../../components/account/transaction/bank-transaction/ApproveTransaction.vue';
export default {
    name: "BankTransactionPage",
    components: {
        NewTransaction,
        EditTransaction,
        DetailTransaction,
    ...
    data() {
        return {
            ...
            addData: {
                type: "",
                bank_ledger: { code: 0, label: "Select from the following" },
                narration: "",
                cheque: "",
                ledgers: [],
                amounts: [],
    ...
    methods: {
        async bankTransactions() {
            ...
            const res = await this.callApi("get", "accounts/transactions/bank-transactions");
            if(res.status == 200)
            {
                this.allTransactions = res.data.bankTransactions;
        ...
        async addTransaction() {
            ...
            if(this.addData.bank_ledger.code == 0)
            {
                return this.$swal({
                    icon: 'error',
                    title: 'Error',
                    text: 'Bank ledger is required',
                });
            }
            if(!this.addData.narration)
            {
                return this.$swal({
                    icon: 'error',
                    title: 'Error',
                    text: 'Bank Narration is required',
                });
            }
            ...
            const res = await this.callApi("post", "accounts/transactions/bank-transactions/add", this.addData);
            if (res.status === 201) {
                this.bankTransactions();
        ...
        async editTransaction(id,type) {
            ...
            const res = await this.callApi("post", "accounts/transactions/bank-transactions/edit", this.voucher);
            if (res.status === 200) {
        ...
        async approveTransaction() {
            const res = await this.callApi("post", "accounts/transactions/bank-transactions/do/approve", this.voucher);
            if (res.status === 200) {
                this.bankTransactions();
                this.addData = JSON.parse(JSON.stringify(this.addDataReset));
                $(".modal").modal('hide');
        ...
        async showTransaction(id,type) {
            this.voucher.id = id;
            this.voucher.type = type;
            const res = await this.callApi("post", "accounts/transactions/bank-transactions/show", this.voucher);
            if (res.status === 200) {
        ...
        async updateTransaction() {
            ...
            if(this.editData.bank_ledger == 0)
            {
                return this.$swal({
                    icon: 'error',
                    title: 'Error',
            ...
            this.btnLoading = true;
            const res = await this.callApi("post", "accounts/transactions/bank-transactions/update", this.editData);
            if (res.status === 200) {
                this.bankTransactions();
                $(".modal").modal('hide');
                this.$swal({
